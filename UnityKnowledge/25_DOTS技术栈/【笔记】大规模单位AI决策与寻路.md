...
sources:
  ...
  - "[[【教程】JobSystem详解]]"
  - "[[【教程】ECS架构入门]]"
related: ["[[【笔记】同屏大规模单位渲染方案]]", "[[【笔记】大规模单位动画方案]]", "[[【实战案例】10w单位渲染与动画最小Demo]]", "[[【教程】JobSystem详解]]", "[[【教程】Burst编译器]]", "[[DOTS专题索引]]"]
---
...
逐对比较 O(N²) 必死。需空间索引加速邻域查询：

| 方案 | 适用 |
|------|------|
| **Uniform Grid**（`NativeMultiHashMap<int3, Entity>`，按格子哈希） | 单位密度均匀，实现简单，Flow Field 同款网格复用 |
| **Unity.Physics broadphase** | 已用 Unity Physics 时，复用其碰撞树 |
| **NativeQuadTree / KD-Tree**（社区实现） | 单位分布稀疏不均 |

查询「最近敌」：把敌对单位按阵营写入 grid，单位在其所在格 + 邻接 8 格 scan（局部查询，O(邻居数)）。
...
- [[【笔记】同屏大规模单位渲染方案]] · [[【笔记】大规模单位动画方案]] · [[【实战案例】10w单位渲染与动画最小Demo]] · [[【教程】JobSystem详解]] · [[【教程】Burst编译器]] · [[【教程】ECS架构入门]]
