...
status: 待验证
validation: Demo验证
related: ["[[【设计原理】GC工作原理深度解析]]", "[[【踩坑记录】内存泄漏模式]]", "[[【实战案例】内存泄漏排查实战]]"]
---
...
Unity各类资源（AssetBundle、Resources、纹理、音频、GameObject）的卸载策略完整指南，涵盖引用计数式AssetBundle管理器、纹理压缩格式与内存占用、音频加载策略、场景切换时的资源清理流程，以及5种常见内存泄漏原因和检查清单。
