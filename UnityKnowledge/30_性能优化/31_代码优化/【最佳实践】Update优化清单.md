...
status: 待验证
validation: Demo验证
related: ["[[【踩坑】常见性能陷阱]]", "[[【笔记】foreach vs for]]", "[[../../30_性能优化/32_内存管理/【最佳实践】GC优化清单]]"]
author: llm
---
...
Update函数性能优化的完整检查清单，按优先级分为P0必须避免（Update中new对象、字符串拼接、LINQ、GetComponent）和P1强烈推荐（对象池、协程替代、事件驱动），并提供Update/FixedUpdate/LateUpdate/协程的选择指南和量化的性能对比数据。
...
**性能提升**：74倍（参见[[../../30_性能优化/32_内存管理/【笔记】对象池vs实例化]]）
