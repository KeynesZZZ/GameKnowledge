...
- `./docs/开发规则清单.md` - 项目本地规则清单（推荐）
- `unity-rules-checker/docs/开发规则清单.md` - 工具包规则清单（备选）
- 本SKILL内置规则摘要 - 离线模式（功能受限）

## 可移植性说明

此SKILL设计为可移植工具，可在任何Unity项目中使用：

1. **自动路径检测**：SKILL会自动在以下位置查找规则清单：
   - 项目根目录 `docs/开发规则清单.md`
   - 工具包目录 `unity-rules-checker/docs/开发规则清单.md`

2. **规则清单集成**：如果规则清单存在，SKILL会读取完整的60条规则

3. **离线模式**：如果找不到规则清单，SKILL仍可使用内置规则摘要进行基本检查

4. **版本管理**：工具包支持版本控制，可随时更新到最新版本
